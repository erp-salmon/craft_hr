...
def reset_leave_allocation():
    ...
    proration_method = getattr(settings, 'earned_leave_proration_method', 'Monthly') or "Monthly"
    reset_to_date = getattr(settings, 'reset_allocation_to_date', None)

    # Get max carry forward per leave type
    leave_types = list(set(a.leave_type for a in allocations))
    max_carry_forward_map = {}
    for lt in frappe.db.get_all(
        'Leave Type',
        filters={'name': ['in', leave_types]},
        fields=['name', 'maximum_carry_forwarded_leaves']
    ):
        max_carry_forward_map[lt.name] = lt.maximum_carry_forwarded_leaves or 0
    ...
    for alloc in allocations:
        ...
        if carry_forward:
            ...
            # Apply max carry forward limit from Leave Type if set
            max_carry_forward = max_carry_forward_map.get(alloc.leave_type, 0)
